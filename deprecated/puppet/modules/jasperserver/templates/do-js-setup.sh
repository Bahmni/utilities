...
initializeAntEnvironment() {
  ...
  then
    ...
    log "Bundled Ant not found. Using existing Ant."
...
#MOTECH CHANGED
#Following line was piping output to log file, which would eat up the return code from ant. Removed log pipe.
#   $ANT_RUN --noconfig -nouserlib -f build.xml $targetName $options 2>&1 | tee -a $JS_LOG_FILE
    $ANT_RUN --noconfig -nouserlib -f build.xml $targetName $options
    antReturnCode=$?
